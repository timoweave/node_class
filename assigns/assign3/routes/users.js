...
var one_routes = router.route("/:id(\\d+)")
                 .get(verify_user, verify_admin, crud.get_one(User, "User", "user_id"))
                 .put(verify_user, verify_admin, crud.update_one(User, "User", "user_id"))
                 .post(verify_user, verify_admin, crud.cannot_do_it(User, "User"))
                 .delete(verify_user, verify_admin, crud.remove_one(User, "User", "user_id"));

var all_routes = router.route("/")
                 .get(verify_user, verify_admin, crud.get_all(User, "User"))
                 .put(verify_user, verify_admin, crud.cannot_do_it(User, "User"))
                 .post(register_user)
                 .delete(verify_user, verify_admin, crud.remove_all(User, "User"));
...
function register_user(request, response, next)
{
    ...
    function add_user(err, user)
    {
        ...
        passport.authenticate('local')(request, response, function () {
            ...
            var data = { "status" : "Registration Successful!" };
            response.json(data);
